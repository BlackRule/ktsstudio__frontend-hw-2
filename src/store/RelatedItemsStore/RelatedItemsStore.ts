...
export default class RelatedItemsStore implements IRelatedItemsStore, ILocalStore {
    get state() {
        return this._state
    }
    ...
    async getItems(params: GetItemsParams): Promise<void> {
        ...
        this._list = []
        const response = await this.productService.getProductsResponse(params.category)
